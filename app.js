(function (S) {
  ...
  async function boot(){
    try{
      ...
      if(app.state.progress?.work?.weaving&&app.state.progress.work.weaving.phase==='PLAYING'&&!app.state.progress.work.weaving.result){
        const session=app.state.progress.work.weaving;
        const c={generation:app.state.meta.generation,revision:app.state.meta.revision,sourceType:'recovery',sourceId:'weaving-abort-'+app.state.meta.revision,type:'WEAVE_ABORT',payload:{sessionId:session.id}};
        app.state=(await store.execute(c)).state;
      }
      S.ui.mount(app);
