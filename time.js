(function (S) {
  ...
  S.time = {
    ...
    describe(p) {
      ...
      const tripDay = trip ? Math.floor((p.world.tick - trip.startedAt) / 3) + 1 : null;
      return { yearLabel: '贞元十六年', dateLabel: S.time.format(p.world.tick).replace('贞元十六年', '').split('·')[0], phaseLabel: ['晨', '午', '暮'][p.world.tick % 3], tripLabel: trip ? '商旅第' + tripDay + '日 / 22日' : '未启程', tripDay, tripTotal: 22, remainingLabel,deadlineLabel:trip?S.time.format(trip.deadlineTick):null };
    },
