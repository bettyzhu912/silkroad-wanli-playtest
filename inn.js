(function (S) {
  ...
  function reduce(p, command, ctx) { E(Object.hasOwn(reducers, command.type), 'UNKNOWN_COMMAND'); const tickBefore = p.world.tick; const result = reducers[command.type](p, command.payload || {}, ctx); const delta = result.operationCashDelta ?? -(result.actionCost || 0); p.journal.push({ type: command.type === 'inn.talk' ? 'innTalk' : ['inn.wait', 'inn.waitCommission'].includes(command.type) ? 'innWait' : command.type === 'inn.camp' && result.kind !== 'eventOpened' ? 'event' : 'inn', action: command.type, amount: delta, tick: tickBefore, tripId: p.trip?.id || null }); return result; }
