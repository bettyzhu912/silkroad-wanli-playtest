...
test('blocking-reason priority helper: 未解锁 → 货位不足 → 铜钱不足 → 数量限制, one reason at a time', () => {
  ...
  assert.strictEqual(ui.blockReason('buy', f), '尚未建立该货源关系');
  assert.strictEqual(ui.blockReason('buy', { ...f, unlocked: true }), '行囊货位不足');
  assert.strictEqual(ui.blockReason('buy', { ...f, unlocked: true, available: 6 }), '随身现钱不足');
  ...
  assert.strictEqual(ui.blockReason('buy', { valid: false, n: 0, unlocked: true, slotCost: 2, available: 1, cash: 100, unit: 10 }), '行囊货位不足', 'empty entry judged as the smallest trade');
  assert.strictEqual(ui.blockReason('sell', { valid: true, n: 3, max: 2 }), '出售件数无效'); assert.strictEqual(ui.blockReason('sell', { valid: true, n: 2, max: 2 }), '');
  assert.strictEqual(ui.blockReason('provisions', { valid: true, n: 5, needsSlot: true, available: 0, cash: 100, unit: 1 }), '补给需要一个货位');
  assert.strictEqual(ui.blockReason('provisions', { valid: true, n: 5, needsSlot: false, available: 0, cash: 3, unit: 1 }), '随身现钱不足');
  assert.strictEqual(ui.blockReason('provisions', { valid: false, n: 0, needsSlot: false, available: 0, cash: 3, unit: 1 }), '请输入正整数日份');
});
console.log(`market exit: ${passed}/4 passed`);
