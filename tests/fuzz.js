...
function simulate(S, seed, STEPS, opts = {}) {
  ...
  function invariants(q) {
    ...
    if (S.commissions.deliveryEligibility) { // COMMISSION v3.0 invariants
      E(q.commissions.active.every(c => ['accepted', 'pending_pickup', 'in_transit', 'ready_to_turn_in'].includes(c.status)), 'V3_ACTIVE_TERMINAL', 'terminal commission left in active');
      for (const c of q.commissions.active) E(c.deadlineWorldTick === c.acceptedAtWorldTick + 90 || c.deadlineMigrated === true, 'V3_DEADLINE', 'active commission without its independent 30-day deadline');
      E(q.commissions.board.length <= 9 && q.commissions.board.every(c => c.status === 'available'), 'V3_BOARD', 'board out of shape');
      E(q.departureDraft === undefined && q.commissions.pool === undefined, 'V3_NO_SECOND_POOL', 'a second candidate pool exists');
    }
  ...
  for (let step = 0; step < STEPS; step++) {
    ...
    const cv = p.work?.caravan;
    if (cv && (cv.phase === 'PLAYING' || cv.result && !cv.settled)) {
      cov('caravan.play');
      if (cv.result) { const before = { cash: p.cash, tick: p.world.tick }; run('CARAVAN_SETTLE', { sessionId: cv.id, settlementId: cv.settlementId }); if (p.work.caravan.settled && p.world.tick !== before.tick) { S.util.ensure(p.cash - before.cash === cv.result.totalWage && p.world.tick - before.tick === 2 && p.world.tick % 3 === 2, 'CARAVAN_SETTLE_INVARIANT', 'settlement must pay once and land on 暮'); cov('caravan.settled'); } continue; }
      if (R() < .15) { run('CARAVAN_ABORT', { sessionId: cv.id }); cov('caravan.abort'); continue; }
      const n = Math.floor(R() * 4), tiers = Array.from({ length: n }, () => ['MODEST', 'NORMAL', 'RICH'][Math.floor(R() * 3)]);
      run('CARAVAN_FINISH', { sessionId: cv.id, outcome: { reason: n === 3 && R() < .7 ? 'ALL_BATCHES_COMPLETED' : 'TIMEOUT', tiers, elapsedSeconds: Math.floor(R() * 76), remainingSeconds: 0 } });
      if (p.work.caravan.result) S.util.ensure(p.work.caravan.result.totalWage <= 22 && (n === 0 ? p.work.caravan.result.totalWage === 0 : p.work.caravan.result.totalWage >= 11), 'CARAVAN_WAGE_RANGE', 'wage out of range');
      continue;
    }
    ...
    if (v) { cov('market.visit'); const r = R(), own = p.inventory.lots.filter(l => l.ownership === 'playerOwned' && !l.nonMarketable && l.condition !== 'destroyed');
      ...
      else if (r < .95) { const c = (p.commissions.board || p.commissions.pool || []).find(x => x.status === 'available'); if (c) run('commission.accept', { commissionId: c.commissionId }); }
    ...
    if (R() < .25) { // directed behaviours
      ...
      const deliverable = here.find(c => { try { return S.commissions.deliveryEligibility ? S.commissions.deliveryEligibility(p, c).ok : S.commissions.eligibleDelivery(p, c, S.core.context('sim')); } catch (_) { return false; } });
