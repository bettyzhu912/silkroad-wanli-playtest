'use strict';
// HUD 五窗口 Round 4 Correction (Round 28) + Round 29 (light tab navigation, transparent tab bar, no mask behind the five windows) — runtime acceptance (headless Chrome, mobile emulation).
...
const MSG = `(()=>{const r=e=>{const b=e.getBoundingClientRect();return {x:+b.x.toFixed(1),y:+b.y.toFixed(1),w:+b.width.toFixed(1),h:+b.height.toFixed(1),right:+b.right.toFixed(1),bottom:+b.bottom.toFixed(1)}};const cs=e=>getComputedStyle(e);const box=document.querySelector('.primary-panel');const body=box.querySelector('.panel-body');
  ...
  const bar=body.querySelector('.msg-tabbar');const tabsRow=body.querySelector('.msg-tabs');const lum=c=>{const m=c.match(/\\d+/g)||[0,0,0];return (+m[0]*299+ +m[1]*587+ +m[2]*114)/1000};
  const sep=e=>{const b=getComputedStyle(e,'::before');return {w:parseFloat(b.width)||0,h:parseFloat(b.height)||0,bg:b.backgroundColor}};
  const sketch=(()=>{const box=document.querySelector('.primary-panel');const q=box.getBoundingClientRect();const sc=parseFloat(getComputedStyle(box).getPropertyValue('--art-scale'))||.46;const x=q.x+q.width*.5+620*sc*0+ (620-384)*sc, y=q.y+215*sc;const e=document.elementFromPoint(x,y);let opaque=false,node=e;while(node&&node!==box){const bg=getComputedStyle(node).backgroundColor;if(bg&&bg!=='rgba(0, 0, 0, 0)'&&!/rgba\(.*, 0\)$/.test(bg))opaque=true;node=node.parentElement;}return {x:+x.toFixed(1),y:+y.toFixed(1),hit:e?e.className||e.tagName:null,coveredByOpaque:opaque}})();
  return {bar:bar?{position:cs(bar).position,rect:r(bar),bg:cs(bar).backgroundColor,stuck:bar.classList.contains('is-stuck')}:null,tabsRow:tabsRow?{underline:parseFloat(cs(tabsRow).borderBottomWidth),underlineColor:cs(tabsRow).borderBottomColor,bg:cs(tabsRow).backgroundColor}:null,separators:[...body.querySelectorAll('.msg-tab')].map(sep),subSeparators:[...body.querySelectorAll('.msg-subtab')].map(sep),tabLum:[...body.querySelectorAll('.msg-tab')].map(t=>lum(cs(t).color)),sketch,tabs:[...body.querySelectorAll('.msg-tab')].map(tab),subtabs:[...body.querySelectorAll('.msg-subtab')].map(tab),view:(body.querySelector('[data-view]')||{}).dataset?.view||null,
  ...
  for (const [w, h, name] of VIEWPORTS) {
    ...
    try {
      ...
      for (const win of WINDOWS) {
        ...
        const bd0 = await ev(`(()=>{const l=document.querySelector('.primary-layer');const b=l.querySelector('.panel-backdrop');const cs=getComputedStyle(b);return {bg:cs.backgroundColor,pe:cs.pointerEvents,cls:l.className,rect:b.getBoundingClientRect().height}})()`);
        check(tag + ' ' + win.title + ' no translucent mask behind / around the window (shared layer backdrop transparent, still blocking)', bd0.bg === 'rgba(0, 0, 0, 0)' && bd0.pe === 'auto' && bd0.cls.includes('art-shell-layer') && bd0.rect > 0, JSON.stringify(bd0));
        ...
        const t = m.typo;
        check(tag + ' ' + win.title + ' typography scale T1 18/700 · T2 15/700 · T3 14/400 · T4 13/400 · T5 14/600 · buttons 14/600', same(t.t1, TYPO.t1) && (!t.t2.length || same(t.t2, TYPO.t2)) && same(t.t3, TYPO.t3) && (!t.t4.length || same(t.t4, TYPO.t4)) && (!t.t5.length || same(t.t5, TYPO.t5)) && (!t.button.length || t.button.every(x => x === TYPO.button[0] + '/' + TYPO.button[1] || x === TYPO.t2[0] + '/700' || x === TYPO.t2[0] + '/400')), JSON.stringify(t));
        if (win.id === 'pack') {
        ...
        if (win.id === 'message') {
          ...
          const tabOk = g.tabs.length === 2 && g.tabs.every(x => x.role === 'tab' && x.border === 0 && x.bg === clear && x.radius === 0 && x.deco === 'none' && x.cursor === 'pointer' && x.rect.h >= 36 && x.shadow === 'none') && near(g.tabs[0].rect.w, g.tabs[1].rect.w, 1) && g.tabs[0].weight === '700' && g.tabs[1].weight === '400' && g.tabs[0].size === TYPO.t2[0] && g.tabLum[0] < g.tabLum[1] && g.tabsRow && g.tabsRow.underline === 1 && g.tabsRow.bg === clear && g.separators[0].w === 0 && g.separators[1].w === 1 && g.separators[1].h > 8 && g.separators[1].h < g.tabs[1].rect.h;
          const subOk = g.subtabs.length === 2 && g.subtabs.every(x => x.role === 'tab' && x.border === 0 && x.bg === clear && x.radius === 0 && x.deco === 'none' && x.rect.h >= 28 && x.rect.h < g.tabs[0].rect.h && x.size < g.tabs[0].size) && near(g.subtabs[0].rect.w, g.subtabs[1].rect.w, 1) && g.subtabs[0].weight === '700' && g.subtabs[1].weight === '400' && g.subSeparators[1].w === 1;
          check(tag + ' 消息 first-level tabs are light top navigation: equal width, no fill / border / radius / shadow, one shared 1 px underline, a short separator between them, selected bold + darker, unselected regular + lighter; sub-tabs same family, smaller; bar sticky', tabOk && subOk && g.bar && g.bar.position === 'sticky', JSON.stringify({ tab0: g.tabs[0], tab1: g.tabs[1], lum: g.tabLum, row: g.tabsRow, sep: g.separators, sub0: g.subtabs[0], subSep: g.subSeparators }));
          check(tag + ' 消息 tab bar transparent at rest, the top-right sketch not covered by any opaque fill (hit ' + g.sketch.hit + ')', g.bar.bg === clear && !g.bar.stuck && g.bodyScrollTop === 0 && !g.sketch.coveredByOpaque, JSON.stringify({ bar: g.bar, sketch: g.sketch }));
          await ev(`(()=>{const b=document.querySelector('.primary-panel .panel-body');b.scrollTop=120;return b.scrollTop})()`); await sleep(200); const sc = await ev(MSG);
          check(tag + ' 消息 tab bar stays on top while scrolled and gets a light paper backdrop only then', sc.bar.position === 'sticky' && sc.bodyScrollTop > 0 && sc.bar.stuck && sc.bar.bg !== clear && sc.bar.rect.y <= g.bar.rect.y + .5, JSON.stringify({ bar: sc.bar, scrollTop: sc.bodyScrollTop }));
          await ev(`(()=>{const b=document.querySelector('.primary-panel .panel-body');b.scrollTop=0;return b.scrollTop})()`); await sleep(200);
