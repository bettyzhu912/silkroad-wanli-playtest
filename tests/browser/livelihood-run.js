...
(async () => {
  ...
  try {
    ...
    check('E 正式结算页：所得工钱 = ' + expectWage + ' 钱（基础 6 + 额外，单行，冻结映射），唯一按钮 结束帮工', frows[4] === '所得工钱=' + expectWage + ' 钱（6+' + (expectWage - 6) + '）' && fbtn === '结束帮工:false', JSON.stringify({ frows, fbtn })); await shot('pc-formal-settle');
