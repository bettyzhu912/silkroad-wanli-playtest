...
(async () => {
  ...
  try {
    ...
    // R39: the Xiaohongshu package carries no audio file (the container's type table accepts none and data: / blob: media are CSP-blocked), so the
    // built stage declares Silk.audioUnavailable and the manager creates no elements. Everything else of v0.1 — settings, persistence, the trigger
    // map, the counters — must behave identically, which is what the AV === false branches assert.
    const AV = st.available !== false; const ELEMS = AV ? 3 : 0;
    console.log('audio assets available in this root: ' + AV);
    check('A init order: manager mounted after the persisted settings, defaults 音乐 开 0.35 / 音效 开 0.7' + (AV ? '; no audio before the first gesture, then the home screen joins the BGM session (one element, loop)' : '; packaged build ships no audio file → no element created, settings intact'), st.mounted && st.prefs.musicEnabled && st.prefs.musicVolume === 0.35 && st.prefs.sfxEnabled && st.prefs.sfxVolume === 0.7 && !st.inGame && st.elements === ELEMS && (AV ? (st.bgm && !st.bgm.paused && st.bgmPlays === 1 && st.bgm.loop) : (st.bgm === null && st.bgmPlays === 0)), JSON.stringify(st));
    await sleep(600); const home0 = await status(); check(AV ? 'A home BGM advancing before the game starts' : 'A home: no BGM element and no playback attempt (silent, no error)', AV ? home0.bgm.currentTime > 0.3 : (home0.bgm === null && !home0.lastError), JSON.stringify(home0.bgm || { bgm: null, lastError: home0.lastError }));
    // ---- home 设置
    ...
    check('C in-game 更多 → 设置: identical state (音乐 关 30% / 音效 关); music (turned off on the home screen) stays off in game, same element', sp && sp.rows.join('|') === '音乐=关闭|音效=关闭' && sp.sliders.join('|') === 'music-volume=30|sfx-volume=70' && st.inGame && st.elements === ELEMS && (AV ? st.bgm.paused : st.bgm === null), JSON.stringify(sp)); await shot('ingame-settings');
    ...
    check('D reload: musicEnabled / musicVolume / sfxEnabled / sfxVolume persisted; music off → BGM never started after the reload (no flash of audio)', pf.musicEnabled === false && pf.musicVolume === 0.3 && pf.sfxEnabled === false && pf.sfxVolume === 0.7 && st.bgmPlays === 0 && (AV ? st.bgm.paused : st.bgm === null), JSON.stringify(pf));
    ...
    check('E 音乐 开 (in game) → ' + (AV ? 'the one BGM element plays, loop on, volume 0.3, time advancing' : 'setting applied, still no element (silent build)'), st.prefs.musicEnabled && st.elements === ELEMS && (AV ? (!st.bgm.paused && st.bgm.loop && Math.abs(st.bgm.volume - 0.3) < 1e-6 && st.bgm.currentTime > 0.3) : st.bgm === null), JSON.stringify(st.bgm || { bgm: null }));
    let last = AV ? st.bgm.currentTime : 0; await closeAll(); const c0 = await counts();
    for (const id of ['pack', 'commission', 'message', 'merchant_business', 'more']) { await ev(`document.querySelector('.hud-tool[data-panel="${id}"]').click()`); await sleep(250); const s = await status(); check('E HUD window ' + id + ': ' + (AV ? 'same element keeps playing, no restart (t ' + last.toFixed(2) + ' → ' + s.bgm.currentTime.toFixed(2) + '), no sfx' : 'no audio element is ever created, no sfx'), s.elements === ELEMS && (AV ? (!s.bgm.paused && s.bgm.currentTime >= last - 0.05 && s.bgmPlays === 1) : (s.bgm === null && s.bgmPlays === 0)), JSON.stringify(AV ? { t: s.bgm.currentTime, plays: s.bgmPlays } : { bgm: null })); last = AV ? s.bgm.currentTime : 0; await closeAll(); }
    await ev(`document.querySelector('.city-hotspot[data-hotspot="market"]').click()`); await busyWait(); await sleep(400); st = await status();
    check('E market open: ' + (AV ? 'BGM continues from ' + last.toFixed(2) + ' (now ' + st.bgm.currentTime.toFixed(2) + '), one instance' : 'still no audio element'), st.elements === ELEMS && (AV ? (!st.bgm.paused && st.bgm.currentTime >= last - 0.05 && st.bgmPlays === 1) : st.bgm === null)); last = AV ? st.bgm.currentTime : 0;
    ...
    const c6 = await counts(); st = await status(); const tBefore = AV ? st.bgm.currentTime : 0;
    ...
    await toggleRow('音乐'); await busyWait(); await sleep(150); st = await status(); const tPaused = AV ? st.bgm.currentTime : 0; await sleep(700); const st2 = await status();
    check('I 音乐 关 → ' + (AV ? 'BGM paused at once, position kept (' + tPaused.toFixed(2) + ')' : 'setting off, nothing to pause'), !st2.prefs.musicEnabled && (AV ? (st.bgm.paused && st2.bgm.paused && Math.abs(st2.bgm.currentTime - tPaused) < 0.05) : st2.bgm === null), JSON.stringify(AV ? { tPaused, t2: st2.bgm.currentTime } : { bgm: null, musicEnabled: st2.prefs.musicEnabled }));
    await toggleRow('音乐'); await busyWait(); await sleep(600); st = await status();
    check('I 音乐 开 → ' + (AV ? 'resumes from the kept position (now ' + st.bgm.currentTime.toFixed(2) + '), same element' : 'setting back on, still silent and error-free'), st.prefs.musicEnabled && st.elements === ELEMS && (AV ? (!st.bgm.paused && st.bgm.currentTime >= tPaused - 0.05 && st.bgm.currentTime < tPaused + 1.5) : (st.bgm === null && !st.lastError)), JSON.stringify(AV ? { tPaused, now: st.bgm.currentTime } : { bgm: null }));
    ...
    check('J music slider drag: ' + (AV ? 'live volume 0.7 applied, ' : '') + 'nothing persisted until release', revMid === revBefore && (AV ? Math.abs(st.bgm.volume - 0.7) < 1e-6 : st.bgm === null), JSON.stringify({ vol: st.bgm ? st.bgm.volume : null }));
    ...
    const bad = (c.network || []).filter(n => n.status !== 200 && n.status !== 304 && n.status !== 206); const audioReq = (c.network || []).filter(n => /audio_bgm_main_v01|sfx_(ui_confirm|coin_gain)_v01/.test(n.url));
    check(AV ? 'Z all requests ok (audio files served)' : 'Z all requests ok; the packaged build requests no audio file at all (nothing to 404)', bad.length === 0 && (AV ? audioReq.length > 0 : audioReq.length === 0), JSON.stringify({ bad: bad.slice(0, 5), audioRequests: audioReq.length }));
