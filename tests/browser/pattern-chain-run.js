...
async function main() {
  ...
  check('FORMAL 所得 row shows 待结算 (C3: no invented cash)', frows[4] === '所得=待结算', frows);
  ...
  check('结束帮工: formal record written (cash null / 待结算), advanceTime(1) → 午, cash unchanged, modal closed', host3.records.length === 1 && host3.records[0].cash === null && host3.records[0].tickBefore === 0 && host3.records[0].tickAfter === 1 && host3.tick === 1 && host3.cash === 100 && (await c.eval('document.getElementById("modal").hidden')), { tick: host3.tick, rec: host3.records[0] && { cash: host3.records[0].cash, before: host3.records[0].tickBefore, after: host3.records[0].tickAfter } });
