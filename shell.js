(function (S) {
  ...
  function renderHUD() {
    ...
    // HUD_POLISH_CORRECTION_v0.2 / v0.3: right slot = time icon (×1.15) + three rows — 年份 (regular, lowest weight) / 月日 · 时辰 (时辰 bold) / 商期　未启程 | 商期　N日 / 22日 (numbers bold, same size as the row). Real runtime calendar labels (S.time.calendar).
    const time=button('',()=>openPanel('time'),{className:'hud-status time-status',label:'查看时间与商期：'+d.yearLabel+d.dateLabel+'·'+d.phaseLabel+'，'+(d.tripDay?'商期第'+d.tripDay+'日，共'+d.tripTotal+'日':'未启程')});const block=el('span','time-block');
    const rowYear=el('span','year-line',d.yearLabel);const rowDate=el('span','date-line');rowDate.append(el('span','date-text',d.dateLabel+' · '),el('b','phase-strong',d.phaseLabel));
    const rowTrip=el('span','trip-row');rowTrip.append(el('span','trip-word','商期'));if(d.tripDay){const v=el('span','trip-value');v.append(el('b','trip-num',d.tripDay+'日'),el('span','trip-sep',' / '),el('b','trip-num',d.tripTotal+'日'));rowTrip.append(v);}else rowTrip.append(el('span','trip-value trip-idle','未启程'));
    block.append(rowYear,rowDate,rowTrip);time.append(icon('time'),block);nodes.hudTop.append(money,rep,time);
