(function (S) {
  ...
  function renderHUD() {
    ...
    // HUD_POLISH_PATCH_v0.1: right slot = two rows — [time icon] 月日 · 时辰 (one line) / 商期　未启程 | 商期　N / 22 日. Real runtime labels only; the constant year stays in the 时间 panel because 「贞元十六年 · 三月十一日起第N日 · 暮」 cannot fit one line in the 50% slot on a 390px-wide phone.
    const time=button('',()=>openPanel('time'),{className:'hud-status time-status',label:'查看时间与商期：'+d.yearLabel+d.dateLabel+'·'+d.phaseLabel+'，'+(d.tripDay?'商期第'+d.tripDay+'日，共'+d.tripTotal+'日':'未启程')});const block=el('span','time-block');const row1=el('span','time-row');row1.append(icon('time'),el('span','date-line',d.dateLabel+' · '+d.phaseLabel));const row2=el('span','trip-row');row2.append(el('span','trip-word','商期'));if(d.tripDay){const v=el('span','trip-value');v.append(el('b','trip-num',String(d.tripDay)),el('span','trip-sep',' / '),el('b','trip-num',String(d.tripTotal)),el('span','trip-unit',' 日'));row2.append(v);}else row2.append(el('span','trip-value trip-idle','未启程'));block.append(row1,row2);time.append(block);nodes.hudTop.append(money,rep,time);
  ...
  function viewportChanged() {
    ...
    if(nodes.root){nodes.root.classList.toggle('keyboard-open',ui.keyboard);nodes.root.style.setProperty('--art-scale',String(Math.max(.3,(nodes.root.clientWidth-24)/768)));
      // HUD_POLISH_PATCH_v0.1: integer-pixel type sizes for the two-row time module, derived from the real HUD height (line 1 = old date size + 1 px, line 2 ≈ line 1 × 1.15).
      {const cs=getComputedStyle(document.documentElement);const safeT=parseFloat(cs.getPropertyValue('--safe-top'))||0,safeB=parseFloat(cs.getPropertyValue('--safe-bottom'))||0;const hudH=(height-safeT-safeB)*.089;const line1=Math.max(8,Math.min(13,Math.round(hudH*.139+1)));const line2=Math.round(line1*1.15);nodes.root.style.setProperty('--hud-line1',line1+'px');nodes.root.style.setProperty('--hud-line2',line2+'px');}const safeBottom=parseFloat(getComputedStyle(document.documentElement).getPropertyValue('--safe-bottom'))||0;nodes.root.style.setProperty('--map-bottom',(Math.max(safeBottom,18)+6)+'px');}fitScene();fitHome();if(wasKeyboard!==ui.keyboard)renderNotices();
