...
- Balance bar: rail 12 px / bead 18 px, bead coloured by the band it sits in, 0.45 s eased movement, clamped so it never overshoots the rail ends. Round-3 follow-up: the bead keeps its previous position across re-renders (the rebuilt element is created at the last position, so it no longer snaps back to the centre first) and the transition lasts 0.2–0.7 s in proportion to the distance moved. `main.thin-bar` keeps the previous 9 px rail for the A/B screenshot (`tests/ab_bar.js` → `tests/results/ab/`).
